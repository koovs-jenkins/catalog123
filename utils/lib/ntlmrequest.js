...
function ntlmRequest(opts) {
  ...
  let SOAPAction =
    opts.body && '"urn:microsoft-dynamics-schemas/page/polist:ReadMultiple"';
  let ContentType = opts.body && 'text/xml; charset="UTF-8"';
  let ntlmState = {
      status: 0,
      data: null
    },
  ...
  function sendRequest(resolve, reject, err, res, body) {
    ...
    if (ntlmState.status === 0) {
      ...
      if (res.statusCode === 401) {
          ...
          if (headerName === "www-authenticate") {
            headerValue.split(",").forEach(authMethod => {
              ...
              if (!/^[^\s=]+=.+/.test(authMethod)) {
                supportedAuthSchemes.push(
                  authMethod.split(/\s/)[0].toLowerCase()
                );
              }
            });
          }
        ...
        if (supportedAuthSchemes.indexOf("ntlm") !== -1) {
          ...
          return reject(
            new Error("Could not negotiate on an authentication scheme")
          );
        }
      } else {
        return resolve({
      ...
      if (res.statusCode >= 400 && res.statusCode <= 499) {
        try {
          ...
          return reject(
            new Error(
              "The server didnt respond properly: " + (e.stack || e.toString())
            )
          );
        }
    ...
    ntlmRequestObj(
      createRequestOptions(authHeader),
      sendRequest.bind(this, resolve, reject)
    );
  }
