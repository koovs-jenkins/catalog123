...
class AllPromotionalPromo extends React.Component {
    state = {
        searched_text :  "",
        selected_filter : "",
        row_data : [],
    ...
    componentDidMount(){
        this.get_promotional_promo_data()
        // this.get_count();
    }

    get_count(){
        var self = this;
        axios.get("/rules/v1/promotion/code/count?genre=generic&type=buyxgety").then(function(res){
            self.setState({ totalElement : res.data.itemCount })
        })
    }

    get_promotional_promo_data(){
        this.props.dispatch(fetchAllPromotionalPromo(this.state.searched_text, this.state.selected_filter, this.state.current_page)).then(() =>
            this.setState({
    ...
    go_to_add(){
        this.props.history.push("/promo/promotional/create")
    }
    ...
    return (
      <React.Fragment>
        ...
        <Grid container lg={12} className={classes.wrapper} justify="space-between">
            ...
            All Promotional Promo 
            </Typography>
            <div className="table_button"><Button onClick={this.go_to_add.bind(this)} variant="contained" color="primary"><Add className="table_icons"/> Promotional Promo </Button></div>
        </Grid>
        
        <Paper className={classes.paper}>
            <Grid container>
                <Grid item xs={12}>
                    <TextField fullWidth variant="outlined" label="Search promotional promo by code" value={searched_text} onChange={this.handlesearch.bind(this)}/>
                </Grid>
            </Grid>
        </Paper>
        <Paper className={classes.paper}>
                            ...
                            row_data.map(function(i,index){
                                return(
                                    <TableRow key={index}>
                                        ...
                                        <TableCell style={{ textAlign :  "left"}}><Link className="edit_button" to={"/promo/promotional/edit/" + i.id}>Edit</Link> </TableCell>
                                    </TableRow>
                                )
                            },this)
                        }
...
const mapStateToProps = state => ({
  ...
  export default withStyles(styles)(connect(mapStateToProps)(AllPromotionalPromo));