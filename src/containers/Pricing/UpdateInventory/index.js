...
import { tagUpdateInventoryCsvHeader } from "../../../../metadata";
import { postUpdateInventoryApi } from "../../../api/pricing";
          ...
          Papa.parse(eventFile, {
            ...
            complete: function(results) {
                if (
                  ...
                  tagUpdateInventoryCsvHeader.join(",")
                ) {
                  that.setState({
                    message: `Found error in header expecting ${tagUpdateInventoryCsvHeader.join(
                      ","
                    )}`
                  });
                } else {
                  ...
                  results.data.map((v, k) => {
                    ...
                    if (
                      ...
                      hasProp(v, "lotId") &&
                      hasProp(v, "operationType") &&
                      hasProp(v, "referenceId") &&
                      hasProp(v, "skuId") &&
                      hasProp(v, "vendorId") &&
                      hasProp(v, "warehouseId")
                    ) {
                      postUpdateInventoryApi(that.props.userId, payload).then(
                        res => {
          ...
          <Grid container spacing={4} style={{height:"80vh"}} alignItems="center" justify="center">
            <Grid item lg={3} >
              ...
              <button
                ...
                onClick={() =>
                  downloadCsv({
                    filename: "UpdateInventory.csv",
                    data: [],
                    header: tagUpdateInventoryCsvHeader
                  })
                }
              >
...
export default withStyles(styles)(connect(mapStateToProps)(UpdateInventory));
