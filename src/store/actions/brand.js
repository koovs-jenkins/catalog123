...
export function fetchAllBrand(searched_text,status,current_page) {
  return dispatch => {
    dispatch(fetchAllBrandBegin());
    return getBrandList(searched_text,status,current_page)
      .then(json => {
        dispatch(fetchAllBrandSuccess(json));
      })
      .catch(error => dispatch(fetchAllBrandFailure(error)));
  };
}
