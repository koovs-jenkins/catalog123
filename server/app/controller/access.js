...
export const asssignedAccess = (req, res,next) => {
  ...
  if ( req.headers["x-api-client"] && req.headers["x-auth-token"] && req.headers["x-user"]  && req.headers["x-user"] != undefined ) {
    pool.getConnection(function(err, connection) {
      ...
      var query = "SELECT role_name,tenantid FROM wms1.roles WHERE is_active=1 and entityid_ref=" + req.headers["x-user"];
      if(env == "local"){
        ...
        function(err, result, fields) {
          ...
          if(roles.length > 0){
            ...
            if(RoleResponse(available_roles,req.headers,module_roles)){
              ...
              res.cookie('tenant', Buffer.from(results[0].tenantid).toString('base64') , { httpOnly: true })
