...
var myTenantId = function (req, res, next) {
  ...
  req.headers["x-tenant-id"] = req.headers["x-tenant-id"] ? req.headers["x-tenant-id"] : "koovs";
  // }
  if (req.cookies["__uid__"]) {
    req.headers["x-ops-id"] = req.cookies["__uid__"]
  }
  console.log("URL ==>", req.url, "x-ops-id ===>", req.headers["x-ops-id"], "x-tenant-id ==>", req.headers["x-tenant-id"])
  next();
}
...
app.use(["/internal/*"], proxy({ logLevel: "debug", target: HomeServiceApiUrl, changeOrigin: true }));
app.use(
  ["/report/*"],
  proxy({ logLevel: "debug", target: localApiUrl, changeOrigin: true })
);
...
app.use(
  ...
  proxy({ logLevel: "debug", target: notificationApiUrl, changeOrigin: true }));
app.use(
